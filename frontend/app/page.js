...
export default function Home(){
 ...
 const [showBook,setShowBook]=useState(false),[showAdmin,setShowAdmin]=useState(false),[showWaiting,setShowWaiting]=useState(false),[showOps,setShowOps]=useState(false),[recoveryAppointment,setRecoveryAppointment]=useState(null),[selected,setSelected]=useState(null),[error,setError]=useState('');
 ...
 async function patch(path,body){const r=await fetch(API+path,{method:'PATCH',headers:{'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined});if(!r.ok) throw new Error(await r.text());return r.json()}
 useEffect(()=>{get('/clinics').then(c=>{setClinics(c);if(c.length&&!clinicId)setClinicId(String(c[0].id))}).catch(e=>setError(e.message))},[refresh]);
 ...
 return <main className="shell">
   <header className="topbar"><div><div className="eyebrow">SPRINT 3G · OPERATIONAL DASHBOARD</div><h1>Clinic Front Desk</h1><p>Appointment operations dashboard</p></div><div className="topActions"><button onClick={()=>setShowOps(true)}>Operational dashboard</button><button onClick={()=>setShowAdmin(true)}>Administration</button><button onClick={()=>setShowWaiting(true)}>Waiting list</button><button className="primary" onClick={()=>setShowBook(true)}>+ New appointment</button></div></header>
   ...
   <section className="filters">
    ...
    <label>Doctor<select value={doctorId} onChange={e=>setDoctorId(e.target.value)}><option value="">All doctors</option>{doctors.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></label>
   ...
   <section className="grid">
    ...
    <aside className="panel attention"><div className="panelHead attentionHead"><div><h2>Attention Queue</h2><small>Highest priority first</small></div><span>{summary?.attention?.length||0}</span></div>
      ...
      {(summary?.attention||[]).length===0?<Empty text="Nothing requires action for this selection."/>:(summary.attention.map((x,i)=><AttentionItem key={`${x.appointment_id}-${i}`} item={x} appointments={appointments} open={setSelected} quickAction={action} recover={setRecoveryAppointment}/>))}
    </aside>
   </section>
   ...
   {showAdmin&&<AdminModal clinicId={Number(clinicId)} clinics={clinics} doctors={doctors} services={services} onClose={()=>setShowAdmin(false)} onChanged={()=>setRefresh(x=>x+1)} setClinicId={setClinicId} setError={setError}/>}
   {showWaiting&&<WaitingListModal clinicId={Number(clinicId)} doctors={doctors} services={services} onClose={()=>setShowWaiting(false)} onChanged={()=>setRefresh(x=>x+1)} setError={setError}/>}
   ...
   {showBook&&<BookingModal clinicId={Number(clinicId)} doctors={doctors} services={services} defaultDoctor={doctorId} defaultDay={day} onClose={()=>setShowBook(false)} onSaved={()=>{setShowBook(false);setRefresh(x=>x+1)}} setError={setError}/>} 
   {selected&&<AppointmentModal a={selected} onClose={()=>setSelected(null)} action={action} onRescheduled={()=>{setSelected(null);setRefresh(x=>x+1)}} setError={setError}/>} 
 </main>
}
...
function AttentionItem({item,appointments,open,quickAction,recover}){
 ...
 return <div className={`attentionCard severity-${item.severity.toLowerCase()}`}>
   ...
   <div className="attentionActions">{item.quick_action==='confirm'&&<button className="primary miniAction" onClick={()=>quickAction(item.appointment_id,'confirm')}>Confirm now</button>}{item.type==='CANCELLED_SLOT'&&a&&<button className="primary miniAction" onClick={()=>recover(a)}>Find candidates</button>}<button className="miniAction" onClick={()=>a&&open(a)}>Open</button></div>
 </div>
}
...
function BookingModal({clinicId,doctors,services,defaultDoctor,defaultDay,onClose,onSaved,setError}){
 ...
 async function json(url,opt){const r=await fetch(API+url,opt);if(!r.ok)throw new Error(await r.text());return r.json()}
...
function AppointmentModal({a,onClose,action,onRescheduled,setError}){
 ...
 useEffect(()=>{Promise.all([fetch(API+`/appointments/${a.id}/risk`).then(r=>{if(!r.ok)throw new Error('Could not load risk');return r.json()}),fetch(API+`/patients/${a.patient_id}/history`).then(r=>{if(!r.ok)throw new Error('Could not load patient history');return r.json()})]).then(([r,h])=>{setRisk(r);setHistory(h)}).catch(e=>setError(e.message))},[a.id,a.patient_id]);
 async function doReschedule(){try{const r=await fetch(API+`/appointments/${a.id}/reschedule`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({start_at:when})});if(!r.ok)throw new Error(await r.text());onRescheduled()}catch(e){setError(e.message)}}
...
function WaitingListModal({clinicId,doctors,services,onClose,onChanged,setError}){
 ...
 async function req(path,opt={}){const r=await fetch(API+path,{...opt,headers:{'Content-Type':'application/json',...(opt.headers||{})}});if(!r.ok)throw new Error(await r.text());return r.json()}
...
function OperationalDashboard({clinicId,doctors,selectedDoctor,selectedDay,onClose,setError}){
 ...
 async function load(){
  try{setLoading(true);setError('');let path=`/dashboard/operational?clinic_id=${clinicId}&start_day=${startDay}&end_day=${endDay}`;if(doctor)path+=`&doctor_id=${doctor}`;const r=await fetch(API+path,{cache:'no-store'});if(!r.ok)throw new Error(await r.text());setData(await r.json())}catch(e){setError(e.message)}finally{setLoading(false)}
 }
